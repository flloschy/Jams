...
def nearestPointOnLines(lines, xM, yM):
    ...
    def cast_all(lines:list, pos:tuple=(0, 0)):
        t = math.pi*2
        points = []
        for angle in np.arange(0, t, t/15):
            angle += math.pi*2 - abs((-1)*(math.pi/180)*360)
            x1, y1 = pos[0], pos[1]
    ...
    def closest(casts):
        ...
        return min(casts, key=lambda x: x[1])[0]
...
def isConnectedToNode(nodes, point):
    closest = min(nodes, key=lambda x: math.sqrt((point.x - x.pos.x)**2 + (point.y - x.pos.y)**2))
    distance = math.sqrt((point.x - closest.pos.x)**2 + (point.y - closest.pos.y)**2)
    return distance <= Settings.nodeCircleSize
...
def isConnectedToNodes(nodes, start, end):
    ...
    for node in nodes:
        ...
        if isConnectedToNode([node], start):
            startNode = node
        if isConnectedToNode([node], end):
            endNode = node
...
def intersectItSelf(points):
    for i, A, B in [(i, x, points[i+1]) for i, x in enumerate(points) if i != len(points) -1]:
        for C, D in [(x, points[i+1]) for i, x in enumerate(points) if i!= len(points) - 1]:
...
class Line:
    ...
    def smoothOut(self):
        if len(self.points) > 3:
            ...
            self.points = [Point(out[0][i], out[1][i]) for i in range(len(out[0]))][::2]
...
class Button:
    ...
    def draw(self, WIN, x, y, l):
        ...
        if x in range(self.pos.x, self.pos.x + self.width) and y in range(self.pos.y, self.pos.y + self.height):
...
while True:
    ...
    if nodes and mode == Mode.doting:
        if all([node.connections == 3 for node in nodes]):
            mode = Mode.end
    ...
    render = font.render("Basic Rules:", True, Settings.normalTextColor)
    ...
    WIN.blit(render, ((Settings.windowWidth - Settings.fieldWidth) / 2 - render.get_width()/2 + Settings.fieldWidth, Settings.fieldHeight / 2 - tutorial.get_height() / 2 - render.get_height() - 10))
