...
class Line:
    ...
    def draw(self, WIN, lines, nodes):
        if not self.done:
            ...
            pygame.draw.lines(WIN, self.color, False, [p.t for p in self.points] + [self._latestXY], 5)
...
while True:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.button == 1 and mode == "placing":
                ...
                mode = "drawing"
                lines.append(Line(x, y, (242, 148, 227) if playerTurn else (148, 212, 242)))
            elif event.button == 3 and mode == "drawing":
                mode = "none"
                lines.remove(lines[-1])
            elif event.button == 1 and mode == "end":
                lines.clear()
                nodes.clear()
    ...
    }[mode].replace("<p>", player1 if playerTurn else player2)

    render = font.render(text, True, (0, 0, 0))
    WIN.blit(render, (800/2 - render.get_width()/2, 5))
    render = font.render(errorMsg, True, (255, 0, 0))
    WIN.blit(render, (800/2 - render.get_width()/2, 800-5-render.get_height()))
    ...
    if everyNodeFull and nodes:
        render = font.render(f"{player1 if playerTurn else player2} won", True, (0, 0, 0))
        for x in range(-3, 3):
            for y in range(-3, 3):
                WIN.blit(render, (800/2 - render.get_width()/2 + x, 800/2 - render.get_height()/2 + y))
        render = font.render(f"{player1 if playerTurn else player2} won", True, (255, 255, 255))
        WIN.blit(render, (800/2 - render.get_width()/2, 800/2 - render.get_height()/2))
        mode = "end"

    render = font.render(f"{player1}: {player1wins}", True, (0, 0, 0))
    WIN.blit(render, (5, 5))
    render = font.render(f"{player2}: {player2wins}", True, (0, 0, 0))
